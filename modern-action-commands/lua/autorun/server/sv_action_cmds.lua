...
local function player_on_timeout(player, command_n)
  ...
  for k,v in _G["pairs"](timeout_players[command_n]) do
...
local function command_exists_in_config(message_cmd)
  for k,v in _G["pairs"](modern_action_commands_config.commands_output) do
...
local function check_command(ply, text, public)
    ...
    if (action_tbl["timeout_amount"] != 0 && player_on_timeout(ply, action_tbl["command"])) then
      _G["net"]["Start"]("modern_timeout")
      _G["net"]["WriteTable"](action_tbl)
      _G["net"]["Send"](ply)
    ...
    _G["net"]["Start"]("modern_action_cmd")
    _G["net"]["WriteTable"](action_tbl)
    _G["net"]["Broadcast"]()
